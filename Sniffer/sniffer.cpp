...
void WriteNetLayer(Packet p){
	...
	outfile << setw(20) << setiosflags(ios::left) << p.time;
	if (p.Protocol == TCP)
	{
		outfile << setw(20) << "TCP";
	}
	...
	outfile << setw(20) << setiosflags(ios::left) << _strupr(Buf);
	outfile << setw(20) << setiosflags(ios::left) << inet_ntoa(p.SrcIp);
	outfile << setw(20) << setiosflags(ios::left) << p.SrcPort;
	memset(Buf, 0, sizeof(Buf));
	sprintf(Buf, "%02x:%02x:%02x:%02x:%02x:%02x", p.DestMac[0], p.DestMac[1], p.DestMac[2], p.DestMac[3], p.DestMac[4], p.DestMac[5]);
	outfile << setw(20) << setiosflags(ios::left) << _strupr(Buf);
	outfile << setw(20) << setiosflags(ios::left) << inet_ntoa(p.DestIp);
	outfile << setw(20) << setiosflags(ios::left) << p.DestPort;
	outfile << setw(20) << setiosflags(ios::left) << p.Length;
